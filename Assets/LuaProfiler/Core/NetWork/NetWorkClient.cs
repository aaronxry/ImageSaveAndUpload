...
namespace MikuLuaProfiler
{
    ...
    public static class LPNetWorkClient
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (m_frameCount != SampleData.frameCount)
                    {
                        ...
                        //写入message 头编号
                        bw.Write((int)0);
                        Sample s = Sample.Create(0, (int)LuaLib.GetLuaMemory(LPLuaProfiler.mainL), "");
                        Serialize(s, bw);
                        s.Restore();
